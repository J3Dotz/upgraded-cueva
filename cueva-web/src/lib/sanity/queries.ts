...
export const ROOM_BY_SLUG_QUERY = groq`
  *[_type == "room" && slug.current == $slug && locale == $locale][0] {
    ...
    sleeps,
    size,
    "view":          coalesce(select(locale == "en" => view.en, view.es)),
    "balcony":       coalesce(select(locale == "en" => balcony.en, balcony.es)),
    "bathroom":      coalesce(select(locale == "en" => bathroom.en, bathroom.es)),
    "materials":     coalesce(select(locale == "en" => materials.en, materials.es)),
    "heating":       coalesce(select(locale == "en" => heating.en, heating.es)),
    amenities,
    "imageUrl": coalesce(image.asset->url, images[0].asset->url),
    "imageAlt": coalesce(image.alt, images[0].alt),
    "gallery":  coalesce(
